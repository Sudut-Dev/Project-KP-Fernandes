...
class LinksController extends Controller
{
    ...
    public function index()
    {
        ...
        $data['schedules'] = Links::orderByDesc('id')->get();
    ...
    public function create()
    {
        ...
        $data['vrooms'] = Vroom::where('status', true)->get();
    ...
    public function store(StoreLinkRequest $request)
    {
        ...
        Links::insert($request->all());


        return redirect()->route('links.index')->with('status', 'Berhasil menambah jadwal peminjaman.');
    }
    ...
    public function edit($id)
    {
        ...
        $data['vrooms'] = Vroom::where('status', true)->get();
        $data['schedule'] = Links::getById($id)[0];
    ...
    public function update(StoreLinkRequest $request, $id)
    {
        ...
        // Insert data pengajuan
        Links::updateById($request->all(), $id);

        // Kirim email ke peminjam yang bersangkutan
        // Mail::send(new ScheduleUpdate($request->all(), auth()->user()->id));

        return redirect()->route('links.index')->with('status', 'Berhasil mengubah jadwal peminjaman.');
    }
    ...
    public function destroy($id, Request $request)
    {
        ...
        return redirect()->route('links.index')->with('status', 'Jadwal telah dibatalkan');
    }
    ...
    public function request()
    {
        ...
        return view('links.request', $data);
    }

    // Proses pengajuan
    public function requestProcess(StoreLinkRequest $request)
    {


        // Insert data pengajuan
        Links::insertRequest($request->all());
    ...
    public function prepare()
    {
        $schedules = [
            [
                ...
                'start' => '09:00',
                'end' => '11:00',
                'description' => 'Rapat Uji Coba Aplikasi A',
                'user_borrower_id' => 3,
                'room_id' => 1,
                'status' => 1,
            ...
            [
                ...
                'status' => 2,
                'requested_at' => now()->subDay(1)->format('Y-m-d H:i:s.u0'),
                'approved_at' => now()->subDay(1)->format('Y-m-d H:i:s.u0'),
            ],
            [
                'date' => now()->addDay(1)->format('Y-m-d'),
                'start' => '14:00',
                'end' => '15:00',
                'description' => 'Rapat Uji Coba Aplikasi B',
                'user_borrower_id' => 3,
                'room_id' => 1,
                'status' => 1,
                'requested_at' => now()->subDay(1)->format('Y-m-d H:i:s.u0'),
